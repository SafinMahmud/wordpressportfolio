    ...
    public function resume_shortcode( $atts, $content = null ){
        ...
        ?><style type="text/css">
            <?php if ( $opts['max_width'] ):
                echo '#resume-' . $rand_rbt; ?>.rb-template-container { max-width: <?php echo $opts['max_width'] . ( $opts['max_width_type'] == 'pixels' ? 'px' : '%' ); ?> !important; margin-left: auto; margin-right: auto; }<?php
            endif; ?>
            <?php echo '#resume-' . $rand_rbt; ?> .rbt-header-wrapper { border-color: <?php echo 'rgba( ' . $border_rgb['r'] . ', ' . $border_rgb['g'] . ', ' . $border_rgb['b'] . ', 0.2 );'; ?> }
            <?php if ( $opts['wrapper_style'] == 'contained' && $opts['container_bg'] ):
                echo '#resume-' . $rand_rbt; ?>.rb-template-container { background: <?php echo $opts['container_bg']; ?> !important; box-sizing: border-box; padding: <?php echo $opts['padding']; ?>px !important; }<?php
            else:
                echo '#resume-' . $rand_rbt; ?>.rb-template-container { background: transparent !important; box-sizing: border-box; padding: 0 !important; }<?php
            endif; ?>
            <?php if ( $opts['wrapper_style'] == 'contained' && $opts['container_shadow'] == 'true' ):
                echo '#resume-' . $rand_rbt; ?>.rb-template-container { box-shadow: 0 30px 50px -30px rgba(0, 0, 0, 0.4) !important; }<?php
            endif; ?>
            <?php if ( $opts['wrapper_style'] == 'contained' && $opts['container_border_radius'] ):
                echo '#resume-' . $rand_rbt; ?>.rb-template-container { border-radius: <?php echo $opts['container_border_radius']; ?>px !important; }<?php
            endif; ?>
            <?php if ( $opts['sidebar_position'] == 'right' ):
                echo '#resume-' . $rand_rbt; ?>.rb-template-container .rbt-body { flex-direction: row-reverse; }<?php
            endif; ?>
            @media screen and (max-width: 900px) {
                <?php echo '#resume-' . $rand_rbt; ?>.rb-template-container {
                    padding:45px !important;
                }
            }
        </style><?php
        
        ?><section id="<?php echo 'resume-' . $rand_rbt; ?>" class="rb-template-container<?php echo ( $compact ? ' rb-compact' : '' ); ?>"><?php
